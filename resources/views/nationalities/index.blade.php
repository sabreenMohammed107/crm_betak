...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page"> {{ __('links.nationalities') }} </li>
    </ol>
  </nav>
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="ms-panel-header d-flex justify-content-between">
			<h6>{{ __('titles.nationalityView') }}</h6>
			<a  href="#" class="btn btn-dark" data-toggle="modal" data-target="#addNationalities"> {{ __('links.add') }}  </a>
		</div>
					...
					<table id="courseEval" class="dattable table table-striped thead-dark  w-100">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $row->name }}</td>
								<td>
									<a href="{{ route('nationalities.edit', $row->id) }}" class="btn d-inline-block btn-info">{{ __('links.edit') }} </a>


									<a href="#" onclick="destroy('this title', '{{$row->id}}')" class="btn d-inline-block btn-danger">{{ __('links.delete') }} </a>

									<form id="delete_{{$row->id}}" action="{{ route('nationalities.destroy', $row->id) }}"  method="POST" style="display: none;">
									@csrf
									@method('DELETE')
                      ...
                      <div class="ms-auth-container row">
                          ...
                          <div class="col-md-12">
                              <div class="form-group">
                                  <label  >{{ __('titles.nationality') }}</label>
